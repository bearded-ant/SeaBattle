...
class MainActivity : Activity() {
        ...
        with(bob) {
            ...
            if (!gameStart) {
                setNegativeButton(android.R.string.cancel) { _, _ ->
                    Toast.makeText(
                        this@MainActivity,
                        getString(R.string.dialog_settings_toast_canceled),
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
    ...
    private fun fillValuesSettings() {
        ...
        // table size
        val editTableSize: EditText =
            settingsDialog!!.findViewById(R.id.dialog_settings_edit_table_size)
    ...
    private fun validityCheckSettings(gameStart: Boolean) {
        ...
        if (valid) {
            Toast.makeText(
                this@MainActivity,
                getString(R.string.dialog_settings_toast_success),
                Toast.LENGTH_LONG
            ).show()
            initGame()
        } else {
            Toast.makeText(
                this@MainActivity,
                getString(R.string.dialog_settings_toast_invalid),
                Toast.LENGTH_LONG
            ).show()
...
//            windowManager.currentWindowMetrics.bounds.height() / tableSize // Landscape Mode - height is limiting factor
        ...
        battleGround2 =
            BattleGround(getString(R.string.battle_ground_player_2), initAvailableShips, tableSize)
    ...
    private fun cellStateWaterGameNotStarted(row: Int, col: Int) {
        ...
        if (currentShip == null) {
            ...
            if ((abs(currentShip!!.endRow - row) + abs(currentShip!!.endCol - col)) != 1) {
                Toast.makeText(
                    this,
                    getString(R.string.activity_main_toast_invalid_cell),
                    Toast.LENGTH_LONG
                ).show()
                return
            }
        ...
        if (currentBattleGround.setShipChecks(currentShip!!)) {
            ...
            Toast.makeText(
                this,
                getString(R.string.activity_main_toast_ship_check_fail),
                Toast.LENGTH_LONG
            ).show()
            resetSelectedShip()
        }
    ...
    private fun cellStateErrorClicked() {
        ...
        Toast.makeText(this, getString(R.string.activity_main_toast_error), Toast.LENGTH_LONG)
            .show()
    }
    ...
    private fun addSelectedShip() {
        ...
        if (currentShip == null) {
            Toast.makeText(
                this,
                getString(R.string.activity_main_toast_no_selection),
                Toast.LENGTH_SHORT
            ).show()
        } else {
            if (currentBattleGround.setShip(currentShip!!)) {
                ...
                Toast.makeText(
                    this,
                    getString(R.string.activity_main_toast_set_ship_fail),
                    Toast.LENGTH_LONG
                ).show()
                resetSelectedShip()
            }
    ...
    private fun resetSelectedShip() {
        ...
        if (currentShip == null) {
            ...
            currentBattleGround.playerBoard[currentShip!!.endRow][currentShip!!.endCol] =
                Cell(CellState.WATER, null)
        } else {
            if (!currentBattleGround.removeShip(currentShip!!)) {
                Toast.makeText(
                    this,
                    getString(R.string.activity_main_toast_reset_ship_fail),
                    Toast.LENGTH_LONG
                ).show()
            }
        }
    ...
    private fun handleWin() {
        ...
        cellState2Color[CellState.SHIP] =
            getColor(R.color.color_cellstate_ship) // loser is allowed to see other ships
        binding.activityMainTvGameState.text =
            getString(R.string.activity_main_tv_game_state_winner, currentBattleGround.playerName)
        changeCurrentBattleGround()
        binding.activityMainButtonNextPlayer.visibility = View.INVISIBLE
